---
sidebar_label: "Prescription Templates"
title: "Prescription Templates Guide for Petaladdin"
description: "Learn how doctors manage prescription templates in Petaladdin to standardize medication instructions and speed up consultation notes."
keywords:
  - "prescription templates"
  - "doctor workflow"
  - "medication instructions"
---


# Prescription Templates

The `Prescription Templates` area lets clinics save reusable medication plans and apply them inside consultations.

![Prescription templates list screen](/img/screenshots/prescription-templates-screen-annotated.jpg)

Quick guide:

- `1` opens `New Template` for a reusable medication plan.
- `2` is the main list area where saved templates are reviewed.
- `3` shows how many medication items are included in each template.
- `4` indicates whether the template is active and available in consultations.
- `5` opens row-level actions such as editing or deactivating the template.

## What users can do here

- review existing templates
- use `New Template`
- edit a template
- activate or deactivate a template

## Template list screen

The page can show:

- `New Template`
- `Edit`
- `Deactivate`

The list screen can show:

- template name
- scope
- item count
- status
- actions

Each template can be scoped by:

- species
- visit type

## Template form

The template form can include:

- template name
- species
- visit type
- diagnosis hint
- status

It also includes reusable medicine rows with:

- medicine type
- medicine name
- dose
- route
- duration
- instructions

![Prescription template form screen](/img/screenshots/prescription-template-form-screen-annotated.jpg)

Quick guide:

- `1` captures the template name staff will recognize during consultations.
- `2` scopes the template to a species when needed.
- `3` adds an optional diagnosis hint to explain when the template should be used.
- `4` inserts additional medicine rows into the reusable treatment plan.
- `5` uses `Save Template` to save the completed template for clinic-wide reuse.

## Why templates matter

Templates save time during consultations and help clinics standardize common treatment plans for repeated conditions.

## Related pages

- [Create Consultation Screen](./create-consultation-screen.md)
- [Edit Consultation Screen](./edit-consultation-screen.md)
