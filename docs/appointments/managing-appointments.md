---
sidebar_label: "Managing Appointments"
title: "Managing Appointments Guide for Petaladdin"
description: "Learn how clinic teams manage Petaladdin appointments from booking and scheduling to status updates, check-in, and follow-up."
keywords:
  - "managing appointments"
  - "appointment scheduling"
  - "clinic visits"
---


# Managing Appointments

Appointments are at the center of day-to-day clinic operations. Petaladdin helps teams schedule, update, and track visits clearly.

## Common tasks

- use `Schedule`
- reschedule an existing visit
- record cancellations or no-shows
- check the daily calendar for doctor availability

## Booking a new appointment

1. Open the appointments or calendar section.
2. Select `Schedule` or choose the required date and time slot.
3. Choose the client and patient.
4. Select the visit type or service.
5. Select `Save Appointment`.

## Rescheduling or cancelling

When a client requests a change:

1. Open the existing appointment.
2. Update the date, time, or assigned doctor as needed.
3. Select `Save Appointment`.
4. If the appointment is cancelled, record the reason when your clinic workflow requires it.

## Best practices

- confirm patient details before saving
- avoid double-booking the same provider or room
- review the daily schedule at the start of each shift
- use consistent cancellation notes for reporting accuracy
